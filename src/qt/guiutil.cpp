...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Voytek address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no Voytek: URI
...
{
    // Convert voytek:// to voytek:
    //
    //    Cannot handle this later, because voytek:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open voytek.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Voytek.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Voytek.lnk
...
{
    return GetAutostartDir() / "voytek.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a voytek.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Voytek\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the voytek app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add voytek app to startup item list
