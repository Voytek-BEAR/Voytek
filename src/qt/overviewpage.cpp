...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeVoytekAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeVoytekAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeVoytekAmount = strAnonymizeVoytekAmount.remove(strAnonymizeVoytekAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVoytekAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeVoytekAmount * COIN) nMaxToAnonymize = nAnonymizeVoytekAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeVoytekAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeVoytekAmount));
        strAnonymizeVoytekAmount = strAnonymizeVoytekAmount.remove(strAnonymizeVoytekAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVoytekAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
