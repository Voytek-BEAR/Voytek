...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("VoytekMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("VoytekMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("VoytekMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("voytek-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running VoytekMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
